			...
			<?php foreach ($categories as $item): ?>
				<li class="nav__item">
					<?php if (isset($item['name'])) : ?>
						<a href="/lot.php?id=<?=htmlspecialchars($item['lot_id']); ?>"><?=htmlspecialchars($item['name']); ?></a>
					<?php endif; ?>
				</li>
			<?php endforeach; ?>
      </ul>
    ...
    <section class="rates container">
      ...
      <table class="rates__list">
		  <?php foreach ($lot_info as $item): ?>
			<tr class="rates__item <?=classToString($item['dt_finish'], $item['winner_id']); ?>">
			  <td class="rates__info">
				<div class="rates__img">
				  <img src="/uploads/<?=htmlspecialchars($item['img']);?>" width="54" height="40" alt="<?=htmlspecialchars($item['lot_name']); ?>">
				</div>
				<h3 class="rates__title"><a href="/lot.php?id=<?=htmlspecialchars($item['lot_id']); ?>"><?=htmlspecialchars($item['lot_name']); ?></a></h3>
				<?php if(isset($item['winner_id'])): ?><p><?=htmlspecialchars($item['contact']); ?></p><?php endif; ?>
			  </td>
